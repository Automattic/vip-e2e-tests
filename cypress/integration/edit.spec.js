describe('WordPress Editor Tests', () => {
  ...
  context('Login ahead of tests', () => {
    ...
    it('can bypass the block editor welcome box.', () => {
      cy.get('.components-button.components-guide__forward-button').click().click().click();
      cy.get('.components-button.components-guide__finish-button.is-primary').click();
    });

    context('Let\'s write a post', () => {
      it('can write a post', () => {
        cy.get('.editor-post-title__input').type('My totally awesome post');
        cy.get('.block-editor-block-list__layout').type('Here is the first paragraph{enter}Here is the second!');
      });
