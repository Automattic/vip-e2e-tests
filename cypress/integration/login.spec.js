  ...
  context('Unauthorized', () => {
    ...
    it('is redirected using cy.request', () => {
      ...
      cy.request({
        ...
        // were redirected to.
        expect(resp.redirectedToUrl).to.eq(`${Cypress.config().baseUrl}/wp-login.php?redirect_to=http%3A%2F%2Flocalhost%3A8000%2Fwp-admin%2F&reauth=1`);
      });
    });
  });
  ...
  context('HTML form submission', () => {
    ...
    it('displays errors on login', () => {
      ...
      cy.get('div#login_error')
        .should('be.visible')
        .and('contain', 'Unknown username. Check again or try your email address.');
